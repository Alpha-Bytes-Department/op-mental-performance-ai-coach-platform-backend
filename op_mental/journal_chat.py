...
class Journal:
    ...
    def __init__(self):
        ...
        self.current_session = {
            ...
            "questions_asked": set()  # Track asked questions to avoid repetition
        }
...
Generate 2-3 specific, actionable insights that are encouraging and personalized, based solely on the evidence sources provided."""
        ...
        try:
            ...
            if response_type == "layer_exploration":
                self.current_session["questions_asked"].add(generated_response[:50])
    ...
    def start_system(self, user_input: str) -> str:
        """Handle system initialization and all user interactions with domain focus."""
        user_input_clean = user_input.strip()
        user_input_upper = user_input_clean.upper()
        
        # Check if user wants to start the system
        if not self.current_session["session_active"] and user_input_upper == "START":
            self.current_session["session_active"] = True
            self.current_session["current_phase"] = "entry_selection"
            self.current_session["session_start"] = datetime.now()
            self.current_session["conversation_history"] = []
            self.current_session["questions_asked"] = set()
            return self._show_entry_options()
        
        # If session not active, prompt for START
        if not self.current_session["session_active"]:
            # Check if the input is coaching-related
            if self._is_coaching_related(user_input_clean):
                return "I'd love to help you explore that! Please type 'START' to begin your personalized coaching session where we can dive deeper into your thoughts."
            else:
                return self._handle_irrelevant_input(user_input_clean) + " Please type 'START' when you're ready to begin."
        
        # Handle different conversation phases
        if self.current_session["current_phase"] == "entry_selection":
            return self._handle_entry_selection(user_input_upper)
        elif self.current_session["current_phase"] == "exploration":
    ...
    def _show_entry_options(self) -> str:
        """Display entry point options for the user."""
        return """

Please choose what you'd like to explore today:

1️⃣ PERSONAL WIN 
2️⃣ PERSONAL CHALLENGE 
3️⃣ PROFESSIONAL WIN 
4️⃣ PROFESSIONAL CHALLENGE 

Please type 1, 2, 3, or 4 to begin your session."""

    def _handle_entry_selection(self, user_input: str) -> str:
        ...
        if user_input in choice_map:
            self.current_session["entry_point"] = choice_map[user_input]
            self.current_session["current_phase"] = "exploration"
            
            opening_message = self.entry_points[choice_map[user_input]]
            self._add_to_history("Selected option " + user_input, opening_message)
            return opening_message
        else:
            return "Please select 1, 2, 3, or 4 to choose your focus area."
    ...
    def _analyze_response(self, response: str) -> None:
        ...
        try:
            ...
            self.current_session["is_future_focused"] = future_similarity > 0.25
    ...
    def export_session_data(self, filepath: str = None) -> str:
        ...
        export_data = {
            "session_metadata": {
                ...
                "session_start": self.current_session["session_start"].isoformat() if self.current_session["session_start"] else None,
